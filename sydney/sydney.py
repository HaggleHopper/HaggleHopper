...
class SydneyClient:
    ...
    def _build_ask_arguments(self, prompt: str, file="") -> dict:
    ...
    async def _ask(
        ...
        # self.conversation_signature = urllib.parse.quote_plus(self.conversation_signature)
        # Create a websocket connection Bing Chat.
        self.wss_client = await websockets.connect(
            BING_CHATHUB_URL+"?sec_access_token="+self.conversation_signature, extra_headers=HEADERS, max_size=None
        )
        # print(BING_CHATHUB_URL+"?sec_access_token="+self.conversation_signature)
        await self.wss_client.send(as_json({"protocol": "json", "version": 1}))
        await self.wss_client.recv()
        
        request = self._build_ask_arguments(prompt,file)
        ...
        while streaming:
            ...
            for obj in objects:
                ...
                if stream and response.get("type") == 1:
                    ...
                    if not messages:
                        continue
                    if messages[-1].get("sourceAttributions",0):
                        for s in messages[-1]["sourceAttributions"]:
                            if s.get("imageLink",0):
                                respFull+=f"Image: `{s['imageLink']}`\n"
                    if messages[-1].get("invocation",0):
                        if "graphic_art" in messages[-1]["invocation"]:
                            p=messages[-1]['invocation'].split("'")[1]
                            respFull+=f"Dalle: `{p}`\n"
                    if raw:
                        yield response, None
                    elif citations:
                ...
                # Handle type 2 messages.
                    ...
                    if not messages:
                        ...
                        elif "User needs to solve CAPTCHA to continue." in obj:
                            if raw:
                                yield "Captcha required", None
                            elif citations:
                                yield "Captcha required", None
                            else:
                                yield "Captcha required", None
                    ...
                    if raw:
                        ...
                        if citations:
                            ...
                            yield respFull, suggested_responses
    ...
    async def start_conversation(self) -> None:
        ...
        # async with session.get("https://edgeservices.bing.com/edgesvc/turing/conversation/create") as response:#BING_CREATE_CONVESATION_URL) as response:
        async with session.get(BING_CREATE_CONVESATION_URL) as response:
            ...
            if response.headers.get("X-Sydney-ConversationSignature",False):
                self.conversation_signature=response.headers.get("X-Sydney-EncryptedConversationSignature")
    ...
    async def ask(
        ...
        async for response, suggested_responses in self._ask(
            prompt, citations, suggestions, raw, stream=False,file=file
        ):
            if suggestions:
    ...
    async def ask_stream(
        ...
        async for response, suggested_responses in self._ask(
            prompt, citations, suggestions, raw, stream=True, file=file,
        ):
            if raw:
                yield response
